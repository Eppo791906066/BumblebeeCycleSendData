...
static uint16 HalUARTWriteDMA(uint8 *buf, uint16 len)
{
  ...
  #if 0 // no use
  {//(Add)Abellee 20140819 Add delay for low baudrate (19200,9600)
    uint16 count;
    count=0xff;
...
void print_msg(const char *fmt, ...)
{
    ...
    buf[1] = '^';
    HalUARTWrite(HAL_UART_PORT, (uint8*)buf, 2);
    #endif
    va_start(args, fmt);
    ...
    for(i=0; i<64; i++)
    {
        ...
        if(buf[i] == '\n')
        {
            ...
            buf2[0] = 0x0d;
            buf2[1] = 0x0a;
            HalUARTWrite(HAL_UART_PORT, (uint8*)buf2, 2);
        }
        else
        {
            HalUARTWrite(HAL_UART_PORT, (uint8*)&buf[i], 1);
        }
    }
}
#endif
