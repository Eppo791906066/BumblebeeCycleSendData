...
#define HAL_UART_Px_RX_TX 0x10 // Peripheral I/O Select for Rx/Tx.
...
void print_msg(const char *fmt, ...)
{
    ...
    buf[1] = '@';
    #if (HAL_UART_ISR == 2)
    HalUARTWrite(HAL_UART_PORT_1, (uint8*)buf, 2);
    #else
    ...
    for(i=0; i<64; i++)
    {
        ...
        if(buf[i] == '\n')
        {
            ...
            buf2[1] = 0x0a;
            #if (HAL_UART_ISR == 2)
            HalUARTWrite(HAL_UART_PORT_1, (uint8*)buf2, 2);
            #else
        ...
        {
            #if (HAL_UART_ISR == 2)
            HalUARTWrite(HAL_UART_PORT_1, (uint8*)&buf[i], 1);
            #else
            HalUARTWrite(HAL_UART_PORT_0, (uint8*)&buf[i], 1);
            #endif
        }
    }
}
